...
void unpack_sksmall(uint8_t rho[SEEDBYTES],
    ...
    for (i = 0; i < L; ++i) {
        smallpolyeta_unpack(s1[i], sk + i * POLYETA_PACKEDBYTES);
    }
    sk += L * POLYETA_PACKEDBYTES;

    for (i = 0; i < K; ++i) {
        smallpolyeta_unpack(s2[i], sk + i * POLYETA_PACKEDBYTES);
    }
    sk += K * POLYETA_PACKEDBYTES;
...
void unpack_sksmall2(uint8_t rho[SEEDBYTES],
    ...
    for (i = 0; i < L; ++i) {
        smallpoly2eta_unpack(s1[i], sk + i * POLYETA_PACKEDBYTES);
    }
    sk += L * POLYETA_PACKEDBYTES;

    for (i = 0; i < K; ++i) {
        smallpoly2eta_unpack(s2[i], sk + i * POLYETA_PACKEDBYTES);
    }
    sk += K * POLYETA_PACKEDBYTES;
